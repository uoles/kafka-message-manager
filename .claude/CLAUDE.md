...
- `index.html` is a self-contained browser UI. It posts JSON to `/api/kafka/send`, performs client-side validation, and stores/renderers message history in browser `localStorage` under `kafkaMessageHistory`; history is not persisted by the server or Kafka. The template also contains the styling and JavaScript, so UI behavior changes generally happen in this file.
