...
- `index.html` is a self-contained browser UI. It posts JSON to `/api/kafka/send`, including optional comma-separated `name=value` Kafka headers, performs client-side validation, and stores/renders message history in browser `localStorage` under `kafkaMessageHistory`; history is not persisted by the server or Kafka. The page uses Bootstrap 5.0.2 tabs: the Send message tab contains the form, while the Message history tab renders a responsive table with Resend controls and headers. Resend repopulates topic, broker, headers, and message, then activates the send tab without sending automatically. The template also contains the page-specific styling and JavaScript, so UI behavior changes generally happen in this file.
